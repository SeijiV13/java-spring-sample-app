...
$(document).ready(function() {
	...
	var itemAdder = function (itemName, itemPrice, itemPriceOrig, quantity, itemDesc, stock, agentname, itemCode, good, warehouseType){
		var trColor = "";
	...
	$('.addToList').click(function(){
		itemAdder($(this).attr('name'), $('#itemAmount').val(), $('#itemAmount2').text(),
				...
				$('#itemCode').text(), $("input[name='good']:checked").val(), $("input[name='warehousetype']:checked").val());
	});
	...
	$('#post-btn').click( function() {
		...
		if(r){
		  ...
		  var currency = "PHP";
		  var details = "{\"refno\" : \""+ refno +"\",\"customer\" : \""+ customer +"\",\"date\" : \""+ date +"\", \"totalAmt\" : \""+ totalAmt +"\",\"currency\" : \""+ currency +"\"}";
		  details = encodeURI(details);
	      $.get('postSalesReturnEntries.htm?request='+tableStr+'&details='+details,function(json) {
	    	  if(json!=null){
	    		  alert(json);
	          }
	          else {
		...
		if(r){
			...
			for (var i = 0; i < table.length ; i++) { 
				...
				itemAdder(table[i]["Description"], $('#itemAmount').val(), $('#itemAmount2').text(),
						...
						table[i]["Item code"], $("input[name='good':checked").val(), $("input[name='warehouseType':checked").val());	
			}
		}
	...
	$('#suspendSales').click( function() {
		...
		if(r){
	      ...
	      $.get('suspendSalesReturnEntries.htm?request='+tableStr+'&details='+details,function(json) {
	    	  if(json!=null){
	    		  alert(json);
	          }
	          else {
